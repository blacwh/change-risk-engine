...
The `insufficient-coverage` rule accepts `minLinePercent` and
`minChangedLinePercent`. Both are finite numbers from 0 through 100 and default
to 80. The rule combines applicable whole-file and changed-line concerns into
one finding weight. Artifact selection is an explicit CLI or Action input rather
than configuration, and the rule emits nothing when complete coverage
